...
function addPhoneNum(){
	if( NumberCheck((frmSMS1.AddReceiveNumber.value).trim().replace(/\-/g,'')) ) 
	{addFormReceiver(document.forms["smsCreateForm1"]); } 
...
function CheckSendForm() {
	...
	var frm1 = document.frmSMS1;
	
	var chkCount = 0;
	frm.RECEIVER_CNT.value = $('input[name="receiverInfo"]').length;
	SelSender = frm1.SELECT_SENDER.options[frm1.SELECT_SENDER.selectedIndex].value;

	if( frm1.SENDER.value.trim() == "" ) {
		if( SelSender == "DIRECT" || SelSender == "") {
			alert("보내는 사람을 입력해 주세요.");
			frm1.SENDER.focus();
			return;
		} else {
			frm1.SENDER.value = SelSender;
		}
	}
	...
	if( document.getElementsByName("scheduleType")[2].checked == true && frm1.REPEAT_TYPE.options[frm1.REPEAT_TYPE.selectedIndex].value == "" ) {
		alert("반복유형을 선택해주세요.");
		frm1.REPEAT_TYPE.focus();
		return;
	}
	...
	if( getByteLength(frm.CONTENT.value.replace(/\r/g, "")) > 80 ) {
		frm1.MSGTYPE.value = "MMS";
	} else {
		frm1.MSGTYPE.value = "SMS";
	}
...
<input type="hidden" name="MSGRES" id="MSGRES" value="N" />
<input type="hidden" name="MSGGRPNO" id="MSGGRPNO" value="0" />
	...
	<div class="col-md-12col-md-6" style="align:center;" >
		<ul class="list-inline" style="text-align:center;" >
			<li>
				<a href="<%=request.getContextPath()%>/ums/smsPopUp" title="문자">
					<i class="fa fa-comments fa-5x"></i>
					<em>간편문자</em>
				</a>
			</li>
			<li>
				<a href="<%=request.getContextPath()%>/smart/smartDmPopUp" title="SMART DM">
					<i class="fa fa-desktop fa-5x"></i>
					<em>간편 DM</em>
				</a>
			</li>
			<li>
				<a href="<%=request.getContextPath()%>/ums/mmsPopUp" title="MMS">	
	...
	<div class="col-md-6 mbox">	
		<div class="mob">
			<div id="CONTENT_BG" class="mbody">
				<textarea name="smsMsg" id="CONTENT"  onkeydown="checkMessageLength(this, 2000, 'SMS', 'spanMessageLength')" onkeyup="checkMessageLength(this, 2000, 'SMS', 'spanMessageLength')"  readonly></textarea>
				<div class="mfoot">
					<em class="mtype sdm">SMART DM</em>
					<em class="mtype sms" id="mtype_sms" style="display:none;">SMS</em>
		...
		<div class="checkbox">
		   <label>
			    <input type="checkbox" name="msgSav"  id="msgSav" value="Y" disabled>메시지 저장
		    </label> 
		    <label>
		    	<input type="checkbox" name="resMode"  id="resMode" value="Y" disabled checked>응답 모드
		    </label>
		</div>
		<div class="btns">
			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					...
					<div class="btns"> 
						...
						<a class="btn btn-second pop-excel" onclick="javascript:goAddrExcelUplodPopup();return false;" >엑셀발송</a>		
					</div>
				</div>
			</div>
		</div>

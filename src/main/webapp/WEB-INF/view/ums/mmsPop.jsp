...
$(document).ready(function() {
 ...
 // 페이지가 로딩될 때 'Loading 이미지'를 숨긴다.
	...
	var defaultText = "보내실 내용을 입력해 주세요. MMS로 처리 및 과금 됩니다.";
	...
	$('#CONTENT').height(140 /*$("#CONTENT_BG").height()-63*/);
});
...
//  $('#upImg').attr("src", 'http://10.10.1.21/upload/image/' + data.file_svr_name).height(160).width(210);
  $('#upImg').attr("src", data.file_full_path + data.file_svr_name).height(160).width(210);
}
...
function fn_MsgSelectAfter(msg_no){
	...
	divSource += "</div>";
	contents.innerHTML = divSource;
  	$('#upImg').attr("src", filepath + fileservername).height(160).width(210);
}
...
<input type="hidden" name="MSGGRPNO" id="MSGGRPNO" value="0" />
	...
	<div class="col-md-6 mbox mms">			
		<div class="mob">
			<div id="CONTENT_BG" class="mbody">
				<div class="uploader">
					<div class="imgarea" id="imgarea" style="margin-bottom:20px;">
						<h1>이미지 영역</h1>
						<a onclick="goImageUploadPopup();" class="pop-img" ><img src="/images/img-upload.png"  id="pop-img" alt="내 PC 이미지 등록" ></a>
					</div>

...
function fn_MsgSelectAfter(msg_no){
	...
	var arrfileservername 	= fileservername.split(";");
	var arrfilepath 		= filepath.split(";");
	var arrfilename 		= filename.split(";");
	var arrfilesize			= filesize.split(";");
	var arrfiletype			= filetype.split(";");
	var arrfilecontenttype	= filecontenttype.split(";");

	contents = document.getElementById('file-canvas');
	contents.innerHTML = "";
	for ( var i =0 ; i < arrfileservername.length-1 ; i++ ) {
		divSource += "<ul class='flist list-unstyled' id='attach_file'>";
		divSource += "  <li id='arr_file_name'>"+arrfilename[i]+"</li>";
		divSource += "  <li id='arr_file_size'>"+arrfilesize[i]+"</li>";
		divSource += "  <li id='arr_content_type'>"+arrfilecontenttype[i]+"</li>";
		divSource += "  <input type='hidden' name='file_name' value='"+arrfilename[i]+"'>";
		divSource += "  <input type='hidden' name='file_size' value='"+arrfilesize[i]+"'>";
		divSource += "  <input type='hidden' name='file_path' value='"+arrfilepath[i]+"'>";
		divSource += "  <input type='hidden' name='file_servername' value='"+arrfileservername[i]+"'>";
		divSource += "  <input type='hidden' name='file_type' value='"+arrfiletype[i]+"'>";
		divSource += "  <input type='hidden' name='file_contenttype' value='"+arrfilecontenttype[i]+"'>";
		divSource += "  <input type='hidden' name='trans_mode' value='I'>";
		divSource += "</ul>";
    }
...
function goFileUploadPopup() {
  ...
  poptarget = window.open("/ums/getRegDocPop"  , title, status);
}
...
function CheckSendForm() {
	...
	fileCount = $('input[name="file_name"]').length;
...
<input type="hidden" name="MSGTYPE" id="MSGTYPE" value="FMS" />
...
<input type="hidden" name="MSGGRPNO" id="MSGGRPNO" value="0" />
	...
	<div class="col-md-4">	
		<div class="panel panel-default fp01">
			<div class="panel-heading">
				<h3 class="panel-title">전송문서</h3>
			</div>
			<div class="panel-body">
				<a onclick="goFileUploadPopup();" class="btn btn-primary pop-img">파일추가</a>
				<!-- <em class="counting">총  개</em>
				 -->
			</div>
			<div id="file-canvas" class="panel-body odd"></div>
			 
		</div>
		<div class="panel panel-default fp02">
			<div class="panel-body">
				<p>스캔가능파일 : bmp, gif, jpg, tif, tiff, 
				doc, docx, xls, ppt, pptx, htm, html, 
				hwp, pdf 
				<em>※ 엑셀2007 문서(xlsx)의 경우 
					하위 버전(xls)으로 변환하여 발송해주세요.</em>
				</p>
				<a href="./pop-fax.html" class="btn btn-second pop-fax">파일추가 유의사항</a>
			</div>
		</div>
	...
	<div class="col-md-4">		
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				<select name="FAX_NUMBER_TYPE" class="form-control" style="display:none;">
					<option value="1">국내팩스</option>
					<!-- <option value="2">국제팩스</option> -->
				</select>

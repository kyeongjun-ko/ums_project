...
//date search
	  ...
	  if($("#srchRegDateEnd").val() == "") {
        ...
        $("#btnSearch").click(function() {
            ...
            $("#transStatListForm").attr({
                action : "/push/pmsTransStatListProc",
                method : "post"
            }).submit();
    ...
    function downloadView() {
        ...
        <form:form method="post" action="/push/pmsTransStatListProc" modelAttribute="searchTransStatVO" id="transStatListForm" >
		...
		<div class="col-md-12 tcon">
			<div class="panel panel-default">
				<div class="panel-heading">
					<h3 class="panel-title">담당자별 / 날짜별 전송 통계 리스트</h3>
				</div>
				...
				<table id="tab01" class="<c:choose><c:when test = "${searchTransStatVO.statType == '01'}">table dtable tabpn</c:when>
					...
					<tr>
						...
						<th style="width: 170px;" class="odd" rowspan="2">담당자명</th>
						<th style="width: 130px;" rowspan="2">발송시도</th>
						<th style="width: 130px;" class="odd" rowspan="2">발송실패</th>
						<th style="width: 130px;" rowspan="2">발송대기</th>
						<th style="width: 130px;" class="odd" rowspan="2">발송성공</th>
						<th colspan="3">PUSH</th>
					</tr>
					<tr>
						<th style="width: 150px;" class="td">TEXT</th>
						<th style="width: 150px;" class="td odd">HTML</th>
						<th style="width: 150px;" class="td">IMAGE</th>
					</tr>
					...
					<c:choose>
						<c:when test="${!empty transStatList && searchTransStatVO.statType eq '01'}">
		                    ...
		                    <tr>	
								...
								<td>${transStatVO.sendTextCnt  }</td>
								<td class="odd">${transStatVO.sendHtmlCnt  }</td>
								<td>${transStatVO.sendImageCnt  }</td>
							</tr>		                        
		                    </c:forEach>    
						</c:when>
						<c:otherwise>
				...
				<table id="tab02" class="<c:choose><c:when test = "${searchTransStatVO.statType == '02'}">table dtable tabpn</c:when>
					...
					<tr>
						<th style="width: 80px;" rowspan="2">No.</th>
						<th style="width: 170px;" class="odd" rowspan="2">날짜</th>
						<th style="width: 130px;" rowspan="2">발송시도</th>
						<th style="width: 130px;" class="odd" rowspan="2">발송실패</th>
						<th style="width: 130px;" rowspan="2">발송대기</th>
						<th style="width: 130px;" class="odd" rowspan="2">발송성공</th>

			...
			return {
				send: function( headers, complete ) {
					...
					callback = function( _, isAbort ) {
						...
						// amazon stg 에서 IE8 로 실행 시, '지정되지 않은 오류' 발생. try/catch 로 넘김.
						try {
							// 2011.07.13 추가. popup 에서 session 이 끊겼을 경우, 부모창 로그인 페이지 이동.
							var sessionExpired = xhr.getResponseHeader("Session-Expired");
							if (sessionExpired === "true") {
								var scriptText = xhr.responseText;
								scriptText = scriptText.replace("<script>","").replace("</script>", "");
								eval(scriptText);
							}
						}
						catch (e) {
						...
						try {
							// 2011.07.14 추가. ajax 요청시. error message 노출.
							var errorHandle = xhr.getResponseHeader("Error-Handle");
							if (errorHandle === "true") {
								var scriptText = xhr.responseText;
								scriptText = scriptText.replace("<script>","").replace("</script>", "");
								eval(scriptText);
